            ...
            node -e "
              ...
              function injectBootstrap(html) {
                const snippet = '<script>(function(){try{' +
                  ...
                  'function localizeStatic(){if(!isZh){return;}var map={\"New session\":\"新建会话\",\"Send\":\"发送\",\"Queue\":\"排队发送\",\"Stop\":\"停止\",\"Connect\":\"连接\",\"Refresh\":\"刷新\",\"Exit focus mode\":\"退出专注模式\"};document.querySelectorAll(\"button\").forEach(function(btn){var raw=\"\";btn.childNodes.forEach(function(node){if(node&&node.nodeType===3){raw+=node.nodeValue||\"\";}});raw=raw.replace(/\\s+/g,\" \").trim();if(map[raw]){replaceFirstTextNode(btn,map[raw]);}var aria=btn.getAttribute(\"aria-label\")||\"\";if(map[aria]){btn.setAttribute(\"aria-label\",map[aria]);}var title=btn.getAttribute(\"title\")||\"\";if(map[title]){btn.setAttribute(\"title\",map[title]);}});document.querySelectorAll(\"textarea\").forEach(function(el){var p=(el.getAttribute(\"placeholder\")||\"\").trim();if(p.indexOf(\"Message\")===0){el.setAttribute(\"placeholder\",\"输入消息（回车发送，Shift+回车换行，可粘贴图片）\");}});document.querySelectorAll(\".muted\").forEach(function(el){var t=(el.textContent||\"\").trim();if(t===\"Loading chat…\"){el.textContent=\"正在加载聊天…\";}});document.querySelectorAll(\".chat-queue__title\").forEach(function(el){var t=(el.textContent||\"\").trim();if(t.indexOf(\"Queued (\")===0&&t.endsWith(\")\")){el.textContent=t.replace(\"Queued (\",\"排队（\").replace(/\\)$/,\"）\");}});document.querySelectorAll(\".chat-new-messages\").forEach(function(el){var t=(el.textContent||\"\").trim();if(t.indexOf(\"New messages\")===0){el.textContent=\"新消息\";}});document.querySelectorAll(\"[aria-label=\\\"Remove queued message\\\"]\").forEach(function(el){el.setAttribute(\"aria-label\",\"移除排队消息\");});}' +
                  ...
                  'function patchNewSession(){if(window.__anyclawNewSessionPatched){return;}var app=document.querySelector(\"openclaw-app\");if(!app||typeof app.handleSendChat!==\"function\"){return;}var original=app.handleSendChat.bind(app);app.handleSendChat=async function(messageOverride,opts){var text=(typeof messageOverride===\"string\"?messageOverride.trim().toLowerCase():\"\");if(text===\"/new\"||text===\"/reset\"){try{if(!this.client||!this.connected){return original(messageOverride,opts);}var nextKey=makeSessionKey(this.sessionKey);await this.client.request(\"sessions.patch\",{key:nextKey,label:\"新会话 \"+new Date().toLocaleString()});var nextUrl=new URL(location.href);nextUrl.searchParams.set(\"session\",nextKey);location.assign(nextUrl.toString());return;}catch(_e){return original(messageOverride,opts);}}return original(messageOverride,opts);};window.__anyclawNewSessionPatched=true;}' +
                  ...
                  'function runPatches(){localizeStatic();patchNewSession();installBackButton();}' +
