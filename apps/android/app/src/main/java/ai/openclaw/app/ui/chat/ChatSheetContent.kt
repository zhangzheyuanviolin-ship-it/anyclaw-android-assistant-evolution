...
import ai.openclaw.app.GatewayAgentOption
...
fun ChatSheetContent(viewModel: MainViewModel) {
  ...
  Column(
    ...
    val quickAgentChoices = remember(gatewayAgentOptions) {
      resolveAgentQuickChoices(gatewayAgentOptions)
    }
    if (quickAgentChoices.isNotEmpty()) {
      AgentQuickSelector(
        choices = quickAgentChoices,
        onSelectAgent = { agentId -> viewModel.selectGatewayAgent(agentId) },
      )
    }
...
@Composable
private fun AgentQuickSelector(
  choices: List<AgentQuickChoice>,
  ...
  Row(
    ...
    for (choice in choices) {
      Surface(
        ...
        color = if (choice.isActive) mobileAccent else mobileCardSurface,
        border = BorderStroke(1.dp, if (choice.isActive) mobileAccentBorderStrong else mobileBorderStrong),
        tonalElevation = 0.dp,
        shadowElevation = 0.dp,
      ) {
        Text(
          text = "${choice.badge} ${choice.title}",
          style = mobileCaption1.copy(fontWeight = if (choice.isActive) FontWeight.Bold else FontWeight.SemiBold),
          color = if (choice.isActive) Color.White else mobileText,
          maxLines = 1,
...
private data class AgentQuickChoice(
  val id: String,
  val title: String,
  val badge: String,
...
private fun resolveAgentQuickChoices(options: List<GatewayAgentOption>): List<AgentQuickChoice> {
  if (options.isEmpty()) return emptyList()
  val preferredNames = listOf("openclaw", "codex", "claude", "opencode", "open code")
  val mapped =
    options.map { option ->
      AgentQuickChoice(
        id = option.id,
  ...
  return mapped.sortedWith(
    compareBy<AgentQuickChoice> {
      val normalized = it.title.lowercase()
      preferredNames.indexOfFirst { key -> normalized.contains(key) }.let { idx ->
