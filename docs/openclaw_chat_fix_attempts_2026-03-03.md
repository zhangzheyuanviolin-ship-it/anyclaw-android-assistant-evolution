...
## Attempt 7
- 时间：2026-03-04
- 假设：Attempt 6 仍失败的根因是“注入脚本本身被转义污染后语法错误”，导致 token 注入逻辑完全未执行。
- 改动：
  - 取消“依赖注入脚本设置 token”的主路径，改为 Control UI 本地服务器在 `/`, `/index.html`, `/chat` 请求时自动补全 `gatewayUrl/simple/token` 查询参数并 302 重定向。
  - 保留后置 UI 本地化脚本，但不再承担鉴权参数注入职责。
  - 修复 `normalizeSpace` 的转义风险写法，避免注入脚本出现非法换行导致 parse error。
- 验证结果：待云端构建后安装验证。
- 结论：回到“URL 参数驱动”的官方兼容路径，规避注入竞态与脚本转义问题。
- 可复用：是（后续所有需要免手动 token 的场景可复用）。
